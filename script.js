...
const { help, donate, islami, downloader, movsto, searching, rantext, ranimage, animanga, infoo, hiburr, makerr, tools, wibuh, nsfww, rohani } = require('./help/help')
...
case 'add':
                  if (isBanned) return reply(`Maaf kamu telah terbanned*\n_hubungi owner saya untuk membuka ban_`)
					...
					if (args.length < 1) return reply(`Yang mau di add siapa? Contoh: ${prefix}add 08xxxxxx`)
...
ranp = getRandom('.gif')
rano = getRandom('.webp')
					anu = await getBuffer(`https://api.xteam.xyz/randomimage/hentaigif?APIKEY=${XteamKey}`, {method: 'get'})
					exec(`wget "${ini_buffer}" -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
	if (err) return reply('Error cok')
						fs.unlinkSync(ranp)
						buffer = fs.readFileSync(rano)
					reply('「❗」KASIH JEDA 1 MENIT')
...
case 'alkitabsong':
         if (isBanned) return reply(`*Maaf kamu telah terbanned*\n_hubungi owner saya untuk membuka ban_`)
         anu = await fetchJson(`http://docs-jojo.herokuapp.com/api/alkitab_songs?q=`)
...
case 'ping':
                ...
                lolhuman.updatePresence(from, Presence.composing) 
                lolhuman.sendMessage(from, `Speed: ${latensi.toFixed(4)} _ms_`, text, { quoted: lol })
                break
case 'donate':
case 'donasi':
                lolhuman.updatePresence(from, Presence.composing) 
...
_hubungi owner saya untuk membuka ban_`)
     ...
     lolhuman.sendMessage(from, lamgu, audio, {mimetype: 'audio/mp4', quoted: lol})
                ...
                if (isBanned) return reply(`*Maaf kamu telah terbanned*
_hubungi owner saya untuk membuka ban_`)
                    if (args.length == 0) return reply(`Contoh: ${prefix + command} Tutorial makan nasi`)
                    query = args.join(" ")
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/ytsearch?apikey=${apikey}&query=${query}`)
                    buffering = get_result.result
                    ini_txt = ""
...
_hubungi owner saya untuk membuka ban_`)
                    if (args.length == 0) return reply(`Contoh: ${prefix + command} https://vt.tiktok.com/ZSwWCk5o/`)
                    ini_url = args[0]
                    ini_url = `https://api.xteam.xyz/dl/tiktok?url=${ini_url}&APIKEY=${xteam}`
                    get_result = await fetchJson(ini_url)
					...
					if ((isMedia && !lol.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								fs.unlinkSync(media)
								reply(`Error, coba ulangi beberapa saat lagi`)
							})
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('BOTWEA', authorname)} ${ran} -o ${ran}`, async (error) => {
									if (error) return reply(`Error, coba ulangi beberapa saat lagi`)
									lolhuman.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: lol})
									fs.unlinkSync(media)	
									fs.unlinkSync(ran)	
								})
							})
